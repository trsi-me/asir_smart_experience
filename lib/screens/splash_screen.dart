...
import 'package:asir_smart_experience/screens/welcome_entry_screen.dart';
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < dotCount; i++) {
      ...
      canvas.drawCircle(
        dotCenter,
        3,
        Paint()..color = dotColor.withValues(alpha: 0.7),
      );
    }
...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _initAndNavigate() async {
    ...
    // إزالة بذرة guest@asir.sa من تثبيتات سابقة قبل أن نقرأ الجلسة المحفوظة.
    await DatabaseHelper.instance.purgeLegacyGuestUser();
    ...
    final next = AuthService().isLoggedIn
        ? const HomeScreen()
        : const WelcomeEntryScreen();
    Navigator.pushReplacement(
      context,
      AppTransitions.fadeScale(next),
    );
  }
