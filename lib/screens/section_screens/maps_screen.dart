...
class MapsScreen extends StatelessWidget {
  ...
  Future<void> _openMapsUrl(String urlStr) async {
    HapticFeedback.lightImpact();
    ...
    return SectionScaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          ...AsirRealContent.all.map((raw) {
            final place = AsirRealContent.toPlaceDetailMap(raw, isArabic: context.isArabic);
            final urlStr = raw['google_maps_url'] as String? ?? '';
            final img = (raw['primary_image'] as String?) ?? AppImages.maps;
            return ImageTile(
              imageUrl: img,
              title: place['name'] as String,
              subtitle: place['subtitle'] as String?,
              onTap: () async => _openMapsUrl(urlStr),
            );
          }),
        ],
      ),
