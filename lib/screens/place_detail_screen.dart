...
import 'package:asir_smart_experience/widgets/content_image.dart';
...
class _PlaceDetailScreenState extends State<PlaceDetailScreen> {
  ...
  Future<void> _checkExistingStamp() async {
    final placeId = widget.place['id'] as String?;
    if (placeId == null) return;
    final has = await DatabaseHelper.instance.hasStampForPlace(
      AuthService().currentUserId,
      placeId,
    );
  ...
  Future<void> _registerVisit() async {
    if (!AuthService().isLoggedIn) {
      await showRegisteredFeatureDialog(
        context,
        context.l10n.guestActionBlocked,
      );
      return;
    }
    final placeId = widget.place['id'] as String?;
    ...
    final allowed =
        await PassportStampPolicy.canCollectStampForPlaceAsync(placeId);
    if (!allowed) {
      if (mounted) {
        setState(() => _checkingStamp = false);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(context.l10n.passportLinkedOnly),
    ...
    final added = await DatabaseHelper.instance.addStamp(
      uid,
      placeId,
      placeName,
      cat,
    ...
    if (mounted) {
      ...
      if (added) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(context.l10n.visitRecordedSuccess),
  ...
  Future<void> _openMaps() async {
    final urlStr = widget.place['google_maps_url'] as String?;
  ...
  void _showAddComment() {
    if (!AuthService().isLoggedIn) {
      showRegisteredFeatureDialog(context, context.l10n.guestActionBlocked);
      return;
    }
              ...
              child: FilledButton(
                onPressed: () {
                  if (c.text.trim().isNotEmpty) {
                    setState(
                      () => _comments.add({'text': c.text.trim(), 'rating': 4}),
                    );
                    Navigator.pop(ctx);
                  }
                },
  ...
  Future<void> _showAddPhoto() async {
    ...
    try {
      ...
      final ext = (name.isNotEmpty && path.extension(name).isNotEmpty)
          ...
          : '.jpg';
      final fileName =
          '${placeId}_${DateTime.now().millisecondsSinceEpoch}$ext';
      final destPath = path.join(photosDir.path, fileName);
      final bytes = await xFile.readAsBytes();
      await File(destPath).writeAsBytes(bytes);
      await DatabaseHelper.instance.addUserPhoto(
        AuthService().currentUserId,
        placeId,
        destPath,
      );
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(context.l10n.photoAdded),
            backgroundColor: AppColors.green,
          ),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(context.l10n.photoAddFailed),
  ...
  Widget build(BuildContext context) {
    ...
    if (placeName == placeId && placeId.isNotEmpty) {
      placeName = place['name'] as String? ?? '';
    }
    var placeSubtitle = dataT.placeSubtitle(placeId);
    if (placeSubtitle.isEmpty) {
      placeSubtitle = place['subtitle'] as String? ?? '';
    }
            ...
            flexibleSpace: FlexibleSpaceBar(
              background: mainImage != null
                  ? Stack(
                      fit: StackFit.expand,
                      children: [
                        ContentImage(path: mainImage, fit: BoxFit.cover),
                        DecoratedBox(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (placeSubtitle.isNotEmpty ||
                      place['subtitle'] != null) ...[
                    const SizedBox(height: 6),
                    Text(
                      placeSubtitle.isNotEmpty
                          ? placeSubtitle
                    ...
                    children: [
                      ...
                      Expanded(
                        child: OutlinedButton.icon(
                          onPressed: () =>
                              showBookingSheet(context, '${place['name']}'),
                  ...
                  SizedBox(
                    width: double.infinity,
                    child: FilledButton.icon(
                      onPressed: (_hasStamp || _checkingStamp)
                          ? null
                          : _registerVisit,
                      icon: _checkingStamp
                          ? const SizedBox(
                          ...
                          : Icon(
                              _hasStamp ? Icons.check_circle : Icons.badge,
                              size: 20,
                            ),
                      label: Text(
                        _hasStamp ? l10n.visitRegistered : l10n.registerVisit,
                      ),
                      style: FilledButton.styleFrom(
                        backgroundColor: _hasStamp
                            ? AppColors.green
                            : AppColors.teal,
                        foregroundColor: AppColors.darkBlue,
                        padding: const EdgeInsets.symmetric(vertical: 14),
                  ...
                  _buildSection(
                    l10n.descriptionHistory,
                    _getDescription(placeId, place, dataT),
                    short: true,
                  ),
                  _buildSection(
                    l10n.whyImportant,
                    _getImportance(placeId, place, dataT),
                    short: true,
                  ),
                  _buildListSection(
                    l10n.activities,
                    _getActivities(placeId, place, dataT),
                  ),
      ...
      child: ListView.separated(
        ...
        itemBuilder: (_, i) => ClipRRect(
          ...
          child: SizedBox(
            width: 140,
            height: 120,
            child: ContentImage(path: images[i], fit: BoxFit.cover),
          ),
        ),
      ),
    );
  ...
  Widget _buildSection(String title, String content, {bool short = false}) {
    ...
    final text = short && content.length > 150
          ...
          (c) => Container(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    ...List.generate(
                      ...
                      (i) => Icon(
                        Icons.star,
                        size: 14,
                        color: i < (c['rating'] as int)
                            ? AppColors.amber
                            : AppColors.white80.withValues(alpha: 0.3),
                      ),
                    ),
                  ],
                ),
                ...
                Text(
                  c['text'] as String,
                  style: TextStyle(fontSize: 14, color: AppColors.white80),
                ),
              ],
            ),
          ),
