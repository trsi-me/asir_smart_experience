...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 7) {
      try {
        await db.execute(
          'ALTER TABLE provider_applications ADD COLUMN activity_photos TEXT',
        );
      } catch (_) {}
    }
    if (oldVersion < 6) {
      try {
        await db.execute('ALTER TABLE provider_applications ADD COLUMN offers TEXT');
      } catch (_) {}
    }
    if (oldVersion < 5) {
      try {
    ...
    if (oldVersion < 2) {
      try {
        await db.execute(
          'ALTER TABLE stamps ADD COLUMN user_id INTEGER DEFAULT 1',
        );
      } catch (_) {}
      try {
        await db.execute(
      ...
      try {
        await db.execute(
          'ALTER TABLE badges ADD COLUMN user_id INTEGER DEFAULT 1',
        );
      } catch (_) {}
      await db.execute(
        'CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, name TEXT NOT NULL, phone TEXT, created_at TEXT NOT NULL)',
      );
      try {
        ...
        if (hasOld.isNotEmpty) {
          await db.execute(
            'CREATE TABLE user_points_new (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER UNIQUE NOT NULL, points INTEGER DEFAULT 0)',
          );
          final old = await db.query('user_points');
          if (old.isNotEmpty) {
            await db.insert('user_points_new', {
              'user_id': 1,
              'points': old.first['points'] ?? 0,
            });
          }
          ...
          await db.execute(
            'CREATE TABLE IF NOT EXISTS user_points (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER UNIQUE NOT NULL, points INTEGER DEFAULT 0)',
          );
        }
      } catch (_) {
        await db.execute(
  ...
  Future<void> _seedDefaultUsers(Database db) async {
    ...
    // الحساب الإداري الوحيد للعرض/المراجعة.
    try {
  ...
  /// ينظّف بيانات بذور قديمة (مثلاً حساب guest@asir.sa) عند الترقية أو التشغيل.
  Future<void> purgeLegacyGuestUser() async {
    ...
    try {
      final rows = await db.query(
        'users',
        where: 'email = ?',
        whereArgs: ['guest@asir.sa'],
      );
      if (rows.isEmpty) return;
      final guestId = rows.first['id'] as int?;
      ...
      for (final tbl in [
        ...
        try {
          await db.delete(tbl, where: 'user_id = ?', whereArgs: [guestId]);
        } catch (_) {}
      }
    } catch (_) {}
  }
  ...
  Future<bool> removeStamp(int? userId, int stampId) async {
    ...
    await db.rawUpdate(
      'UPDATE user_points SET points = MAX(0, points - 10) WHERE user_id = ?',
      [userId],
    );
    return true;
  }
  ...
  Future<int> insertProviderApplication(Map<String, dynamic> row) async {
    final db = await _db;
    return db.insert('provider_applications', row);
  }
  ...
  Future<List<Map<String, dynamic>>> getProviderApplicationsPending() async {
    ...
    final list = await db.query(
      'provider_applications',
      where: 'status IN (?, ?)',
      whereArgs: ['pending', 'needs_revision'],
      orderBy: 'created_at DESC',
    );
    return list.map((e) => Map<String, dynamic>.from(e)).toList();
  }
