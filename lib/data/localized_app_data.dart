...
class LocalizedAppData {
  ...
  /// صيغة عربية للطقس يفهمها خادم `/api/recommend/*` عند كون واجهة المستخدم إنجليزية.
  String get weatherConditionForApi {
    if (weatherData.isEmpty) return 'مشمس';
    final raw = weatherData.first['condition']?.toString().trim() ?? '';
    if (raw.isEmpty) return 'مشمس';
    if (isArabic) return raw;
    final lower = raw.toLowerCase();
    if (lower.contains('fog')) return 'ضباب';
    if (lower.contains('cloud')) return 'غائم';
    if (lower.contains('sun')) return 'مشمس';
    return raw;
  }
