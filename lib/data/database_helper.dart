...
/// قاعدة بيانات محلية - مستخدمين وجواز عسير
class DatabaseHelper {
  ...
  Future<List<Map<String, dynamic>>> getStamps(int? userId) async {
    ...
    final db = await database;
    return await db.query('stamps', where: 'user_id = ?', whereArgs: [userId], orderBy: 'acquired_at DESC');
  }
  ...
  Future<List<Map<String, dynamic>>> getTrips(int? userId) async {
    if (userId == null) return [];
    final db = await database;
    return await db.query('trips', where: 'user_id = ?', whereArgs: [userId], orderBy: 'date DESC');
  }
  ...
  /// حذف ختم (سحب زيارة) - يُسحب معه استحقاق الشهادة إن قلّت الزيارات عن ٣
  ...
  /// يرجع true إذا تم إضافة ختم جديد، false إذا كان موجوداً
  ...
  Future<List<Map<String, dynamic>>> getBookings(int? userId) async {
    if (userId == null) return [];
    final db = await database;
    return await db.query('bookings', where: 'user_id = ?', whereArgs: [userId], orderBy: 'created_at DESC');
  }
